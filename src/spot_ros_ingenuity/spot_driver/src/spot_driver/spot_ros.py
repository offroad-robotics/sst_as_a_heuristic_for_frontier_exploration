...
from geometry_msgs.msg import TwistWithCovarianceStamped, Twist, Pose, PointStamped
...
from spot_msgs.msg import TrajectoryBodyAction, TrajectoryBodyResult, TrajectoryBodyFeedback
from spot_msgs.msg import MaplessTrajectoryAction, MaplessTrajectoryResult, MaplessTrajectoryFeedback
...
class SpotROS():
    ...
    def RobotStateCB(self, results):
        ...
        if state:
            ...
            # Odom #
            # Publishing both sensor readings so they can be fused. 
            odom_vis_msg = GetOdomFromState(state, self.spot_wrapper, use_vision=True)
            self.odom_vis_pub.publish(odom_vis_msg)
    ...
    def WorldObjectCB(self, results):
        """Callback for when the Spot Wrapper gets new world object data
        ...
        if data:
            world_obj_msg = GetWorldObjectsFromState(data, self.spot_wrapper)

            self.world_objects.publish(world_obj_msg)
    ...
    def LocalGridCB(self, results):
        ...
        if data:
            local_grid_tf = self.tf2_buffer.lookup_transform(self.mc_localized_frame, self.mc_spot_frame, rospy.Duration(0))
            # terrain grid
            terrain_grid_msg = GetLocalGridsFromState(data[0], self.spot_wrapper, local_grid_tf)
            self.terrain_grid.publish(terrain_grid_msg)
            # valid grid
            valid_grid_msg = GetLocalGridsFromState(data[1], self.spot_wrapper, local_grid_tf)
            # valid terrain
            ...
            # step grid
            step_grid_msg = GetLocalGridsFromState(data[3], self.spot_wrapper, local_grid_tf)
            self.step_grid.publish(step_grid_msg)
            # obstacle grid
            obstacle_grid_msg = GetLocalGridsFromState(data[4], self.spot_wrapper, local_grid_tf)
    ...
    def handle_mapless_trajectory(self, req):
        """ROS actionserver execution handler to move the robot to a specified location that is localized off a ROS
        frame instead of a Spot internal/saved map
        """

        # abort action if duration hasn't been set properly
        if req.duration.data.to_sec() <= 0:
            self.mapless_trajectory_server.set_aborted(MaplessTrajectoryResult(False, 'Duration must be larger than 0', 0))
            return

        # abort action if wrong frame id is specified. This could cause a crash if improperly set
        if req.target_pose.header.frame_id != self.mc_localized_frame:
            self.mapless_trajectory_server.set_aborted(
                MaplessTrajectoryResult(False, 'Header frame ID does not match localized frame specified', 0))
            return

        # keep track of attempts made
        attempt = 0

        # get translation from global frame to local frame (spot body frame)
        try:
            local_trans = self.tf2_buffer.lookup_transform(self.mc_spot_frame,
                                                           self.mc_localized_frame,
                                                           rospy.Time.now(),
                                                           rospy.Duration(3))
        except:
            rospy.logerr(f"Couldn\'t get transform between {self.mc_spot_frame} and {self.mc_localized_frame} frames")
            self.mapless_trajectory_server.set_aborted(
                MaplessTrajectoryResult(False, f"Couldn\'t get transform between {self.mc_spot_frame} and {self.mc_localized_frame} frames" , 0))
            return

        # convert error to local frame
        local_goal_pose = tf2_geometry_msgs.do_transform_pose(req.target_pose, local_trans)

        inRange = GetInRange(local_goal_pose.pose, self.mc_range, self.mc_radian)

        def timeout_cb(event=None):
            self.trajectory_attempt = False

        while((not inRange) and attempt < self.mc_max_attempts):
            # send robot to goal
            ...
            self.trajectory_attempt = True

            resp = self.spot_wrapper.trajectory_body_cmd(
                goal_x=local_goal_pose.pose.position.x,
                goal_y=local_goal_pose.pose.position.y,
                goal_heading=math_helpers.Quat(
                    w=local_goal_pose.pose.orientation.w,
                    x=local_goal_pose.pose.orientation.x,
                    y=local_goal_pose.pose.orientation.y,
                    z=local_goal_pose.pose.orientation.z
                ).to_yaw(),
                cmd_duration=cmd_duration.to_sec(),
                precise_position=req.precise_positioning,
            )
            ...
            # Abort the actionserver if cmd_duration is exceeded - the driver stops but does not provide feedback to
            # indicate this so we monitor it ourselves
            attempt_timeout = rospy.Timer(cmd_duration, timeout_cb, oneshot=True)

            # The trajectory command is non-blocking but we need to keep this function up in order to interrupt if a
            # preempt is requested and to return success if/when the robot reaches the goal. Also check the trajectory_attempt to
            # monitor whether the timeout_cb has already aborted the command. Also continuously check if we are in range.
            rate = rospy.Rate(10)

            while not rospy.is_shutdown() and not self.mapless_trajectory_server.is_preempt_requested() and self.trajectory_attempt and not inRange:
                # update Feedback and local_goal_pose

                self.mapless_trajectory_server.publish_feedback(MaplessTrajectoryFeedback(local_goal_pose.pose, "attempting goal", attempt))

                try:
                    # uses current time, not the time the request was sent
                    local_trans = self.tf2_buffer.lookup_transform(self.mc_spot_frame,
                                                                   self.mc_localized_frame,
                                                                   rospy.Time.now(),
                                                                   rospy.Duration(1))
                except Exception as e:
                    # f"Couldn\'t get transform between {self.mc_spot_frame} and {self.mc_localized_frame} frames"
                    self.mapless_trajectory_server.set_aborted(MaplessTrajectoryResult(False, e, 0))
                    return
                
                local_goal_pose = tf2_geometry_msgs.do_transform_pose(req.target_pose, local_trans)

                # update inRange and attempt
                inRange = GetInRange(local_goal_pose.pose, self.mc_range, self.mc_radian)

                rate.sleep()

            if not self.trajectory_attempt:
                self.mapless_trajectory_server.publish_feedback(MaplessTrajectoryFeedback(local_goal_pose.pose, "Failed to reach goal on attempt, retrying...", attempt))

            else:
                attempt_timeout.shutdown()

            if self.mapless_trajectory_server.is_preempt_requested():
                self.mapless_trajectory_server.publish_feedback(
                    MaplessTrajectoryFeedback(local_goal_pose.pose, "Preempted", attempt))
                self.mapless_trajectory_server.set_preempted()
                self.spot_wrapper.stop()

        # Success
        if inRange:
            self.mapless_trajectory_server.publish_feedback(MaplessTrajectoryFeedback(local_goal_pose.pose, "Reached localized goal", attempt))
            self.mapless_trajectory_server.set_succeeded(MaplessTrajectoryResult(True, "success", attempt))
            return

        # Failure after multiple attempts
        else:
            self.mapless_trajectory_server.set_aborted(MaplessTrajectoryResult(False, "failure", attempt))

    def handle_body_trajectory(self, req):
        """ROS actionserver execution handler to handle receiving a request to move to a location based on body frame"""
        if req.target_pose.header.frame_id != 'body':
            self.trajectory_server.set_aborted(TrajectoryResult(False, 'frame_id of target_pose must be \'body\''))
            return
        if req.duration.data.to_sec() <= 0:
        ...
        def timeout_cb(trajectory_body_server, _):
            trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Failed to reach goal, timed out"))
            trajectory_body_server.set_aborted(TrajectoryBodyResult(False, "Failed to reach goal, timed out"))

        # Abort the actionserver if cmd_duration is exceeded - the driver stops but does not provide feedback to
        # indicate this so we monitor it ourselves
        cmd_timeout = rospy.Timer(cmd_duration, functools.partial(timeout_cb, self.trajectory_body_server), oneshot=True)

        # The trajectory command is non-blocking but we need to keep this function up in order to interrupt if a
        # preempt is requested and to return success if/when the robot reaches the goal. Also check the is_active to
        # monitor whether the timeout_cb has already aborted the command
        rate = rospy.Rate(10)
        while not rospy.is_shutdown() and not self.trajectory_body_server.is_preempt_requested() and not self.spot_wrapper.at_goal and self.trajectory_body_server.is_active():
            if self.spot_wrapper.near_goal:
                if self.spot_wrapper._last_trajectory_command_precise:
                    self.trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Near goal, performing final adjustments"))
                else:
                    self.trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Near goal"))
            else:
                self.trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Moving to goal"))
            rate.sleep()

        # If still active after exiting the loop, the command did not time out
        if self.trajectory_body_server.is_active():
            cmd_timeout.shutdown()
            if self.trajectory_body_server.is_preempt_requested():
                self.trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Preempted"))
                self.trajectory_body_server.set_preempted()
                self.spot_wrapper.stop()

            if self.spot_wrapper.at_goal:
                self.trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Reached goal"))
                self.trajectory_body_server.set_succeeded(TrajectoryBodyResult(resp[0], resp[1]))
            else:
                self.trajectory_body_server.publish_feedback(TrajectoryBodyFeedback("Failed to reach goal"))
                self.trajectory_body_server.set_aborted(TrajectoryBodyResult(False, "Failed to reach goal"))
    ...
    def main(self):
        ...
        self.local_grid_static_transform_broadcaster = tf2_ros.StaticTransformBroadcaster()
        ...
        if self.spot_wrapper.is_valid:
            ...
            self.odom_vis_pub = rospy.Publisher('odometry_vis', Odometry, queue_size=10)
            ...
            # World Object Publisher
            self.world_objects = rospy.Publisher('status/world_objects', AprilTagState, queue_size=10)
            ...
            self.trajectory_body_server = actionlib.SimpleActionServer("trajectory_body", TrajectoryBodyAction,
                                                                  execute_cb=self.handle_body_trajectory,
                                                                  auto_start=False)
            self.trajectory_body_server.start()

            self.mapless_trajectory_server = actionlib.SimpleActionServer("mapless_trajectory", MaplessTrajectoryAction,
                                                                       execute_cb=self.handle_mapless_trajectory,
                                                                       auto_start=False)
            self.mapless_trajectory_server.start()
            ...
            # Mapless controller parameters
            self.mc_localized_frame = rospy.get_param('~mapless_controller/localized_frame', 'odom')
            self.mc_spot_frame = rospy.get_param('~mapless_controller/spot_frame', 'body')
            self.mc_max_attempts = rospy.get_param('~mapless_controller/max_attempts', 6)
            self.mc_range = rospy.get_param('~mapless_controller/success_range', 0.3)
            self.mc_radian = rospy.get_param('~mapless_controller/success_rads', 0.088)
            self.trajectory_attempt = True
