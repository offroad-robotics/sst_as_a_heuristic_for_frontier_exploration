...
world_object_types = [world_object_pb2.WORLD_OBJECT_APRILTAG]
"""List of types of world objects to recognize"""
...
class AsyncWorldObject(AsyncPeriodicQuery):
    """Class to get world objects at regular intervals.  get_world_objects query sent to the robot at every tick.  Callback registered to defined callback function.
    ...
    def __init__(self, client, logger, rate, callback, world_object_request):
        super(AsyncWorldObject, self).__init__("world-object", client, logger, period_sec=1.0 / max(rate, 1.0))
        self._callback = None
        if rate > 0.0:
            self._callback = callback

        self._world_object_request =  world_object_request

    def _start_query(self):
        if self._callback:
            callback_future = self._client.list_world_objects_async(object_type=self._world_object_request)
            callback_future.add_done_callback(self._callback)
            self._callback(callback_future)
            return callback_future
...
class SpotWrapper():
    ...
    def __init__(self, username, password, hostname, logger, estop_timeout=9.0, rates = {}, callbacks = {}):
        ...
        self._world_object_requests = []
        for object in world_object_types:
            self._world_object_requests.append(object)
        ...
        if self._robot:
            ...
            self._world_object_task = AsyncWorldObject(self._world_object_client, self.logger, max(0.0, self._rates.get("world_object", 0.0)), self._callbacks.get("world_object", lambda:None), self._world_object_requests)

            self._estop_endpoint = None

            self._async_tasks = AsyncTasks(
                [self._robot_state_task, self._robot_metrics_task, self._lease_task, self._front_image_task, self._side_image_task, self._rear_image_task, self._idle_task, self._local_grid_task, self._world_object_task])
    ...
    def world_objects(self):
        """Return latest world_objects from _world_object_task"""
    ...
    def trajectory_body_cmd(self, goal_x, goal_y, goal_heading, cmd_duration, precise_position=False):
        """Send a trajectory motion command to the robot inh body frame.
        ...
        response = self._robot_command(
                        RobotCommandBuilder.synchro_trajectory_command_in_body_frame(
                            goal_x_rt_body=body_tform_goal.x,
                            goal_y_rt_body=body_tform_goal.y,
                            goal_heading_rt_body=body_tform_goal.rot.to_yaw(),
                            frame_tree_snapshot=self._robot_state_client.get_robot_state().kinematic_state.transforms_snapshot,
                            params=self._mobility_params),
                        end_time_secs=end_time
                        )

        # reponse[True, success, id]
        # reponse[False, Error e, None]
