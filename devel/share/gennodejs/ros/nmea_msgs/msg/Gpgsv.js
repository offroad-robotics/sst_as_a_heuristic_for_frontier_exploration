...
class Gpgsv {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.satellites.forEach((val) => {
      bufferOffset = GpgsvSatellite.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.satellites[i] = GpgsvSatellite.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.satellites !== undefined) {
      ...
      for (let i = 0; i < resolved.satellites.length; ++i) {
        resolved.satellites[i] = GpgsvSatellite.Resolve(msg.satellites[i]);
      }
    }
    else {
      resolved.satellites = []
